...
## 라이브러리 추가 (2026.04.23)
### Python
- 행정동 경계를 보다 손쉽게 이용할 수 있는 파이썬 라이브러리를 추가했습니다.
  - lib 폴더의 readme에 간단히 소개했습니다.
  - examples 폴더의 example.ipynb 를 구글 코랩에서 바로 실행해볼 수 있습니다.
- 1975년 이후의 읍면동/시군구/시도 경계까 parquet 형식으로 추가했습니다.
  - 1975~2000년까지는 5년 단위 입니다. 국가데이터처에서 내려받은 파일을 이용해서 만들었습니다.
  - 1990년부터는 10자리 행정동 코드도 매칭되어 있습니다.
  - 컬럼 이름 등을 같은 기준으로 만들었습니다.
  - EPSG:5179 좌표계입니다.파이썬 라이브러리를 이용해서 EPSG4326(경위도 좌표계)로 변환하여 받을 수 있습니다. 자세한 내용은 lib 폴더의 readme를 참고하세요.
- 파이썬 라이브러리에서 get으로 지도를 불러올 때 이 github의 parquet 지도들을 불러오게 됩니다.
- 파이썬 라이브러리는 다음과 같은 목적으로 만들었습니다.
  - 특정 시점의 읍면동/시군구/시도 불러오기
  - 특정 시점의 행정구역 영역 기준으로 매칭되는 과거 시점의 행정구역들 불러오기 (시계열 변화 탐지)
  - 두 시점을 비교하여 행정구역 차이점 알아보기
- 아직 v0.5.0이라 실제 사용하면서 다른 기능이 추가될 수 있습니다.
  - 사용 후 개선에 대한 건의 사항이 있다면 이 Github의 Issue 게시판에 올려주세요.

### Javascript
 - 자바스크립트 라이브러리 역시 파이썬에서 제공하는 기능을 거의 유사하게 제공합니다.
 - 분석 플랫폼이 아니므로, get 함수로 지도를 가져올 때 기본적으로 단순화된 버젼을 가져오도록 했습니다.
 - 즉, 프론트엔드에서 여러 시기의 지도를 곧바로 호출하여 렌더링할 수 있습니다.
 - js 폴더에 라이브러리가 있습니다. npm install admdongkor 이후 import 해서 사용가능합니다. 
 - web 폴더에는 데모 페이지가 있습니다.
 - 데모 페이지는 다음의 주소에서 직접 조작해 볼 수 있습니다.
 - https://admdongkor.vw-lab.com
 - 자세한 내용은 lib 폴더의 readme 를 참고하세요.
---
