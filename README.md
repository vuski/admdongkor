...
- 바로 아래 항목에서 언급하는, 도로명주소지도 사이트의 행정동 지도 월별 공개 사실을 알게 되어 이 깃헙은 업데이트를 종료하거나 다른 형식으로 대체할 예정입니다.
- 기존에는 국가기관에서 제공하던 행정동 경계의 업데이트가 매우 늦어 이 깃헙을 운영하게 되었으나, 도로명주소지도의 경우 매달 업데이트 되므로 굳이 덜 자세한 지도를 수작업하여 배포할 필요가 없다고 판단됩니다.
- 따라서, 이 깃헙은 도로명주소지도 사이트에서 제공하는 행정동경계의 정합성등을 수정하는 코드 등의 게시 용도로 사용하거나, 다른 특별한 경우에만 업데이트 하는 식으로 사용할 예정입니다.
- 임시로 업데이트 하였습니다.(2024.09.25).
