...
행정동 경계를 보다 손쉽게 이용할 수 있도록 파이썬·자바스크립트 라이브러리를 함께 제공합니다. 지도 데이터(parquet) 와 검색·시계열 매칭 인덱스는 이 저장소의 내용을 그대로 가져다 씁니다.

### 데모 페이지

- 웹에서 연도별 경계를 직접 조작해 볼 수 있습니다: **<https://admdongkor.vw-lab.com>**
- 연도 슬라이더로 1975 ~ 2026 시점을 이동하며 읍면동 경계 변화를 확인할 수 있습니다.
- 각 시점의 지도를 여러가지 좌표계로 다운받을 수 있습니다. 파일 포맷, 단순화 정도도 조정 가능합니다. 백령도, 연평도, 울릉도, 독도, 흑산도, 제주도를 보조선과 함께 육지 가까이 당겨오는 버젼도 제공합니다.

### Python (PyPI: `admdongkor`)
...
- 주요 기능:
  ...
  - `find()` — 행정구역명 또는 **코드**로 시점 검색 (이름은 공백 토큰 수로 시도·시군구·읍면동 자동 추정, 숫자만 넣으면 코드 검색)
  - `find_offices()` — 출장소 코드 조회 (경계 지도가 없어 코드 체계에만 존재하는 392건)
  - `match_adm()` — 특정 시점 영역 기준으로 다른 시점의 행정구역들을 면적 가중으로 매칭 (시계열 변화 탐지·인구 재집계)
  - `compare()` — 두 시점을 비교하여 경계가 바뀐 / 소멸 / 신설된 읍면동 추출

### JavaScript (npm: `admdongkor`)

- 설치: `npm install admdongkor`
- 자세한 사용법은 [js/README.md](js/README.md) 를 참고하세요.
- 파이썬과 거의 동일한 API 를 제공합니다. 분석 플랫폼이 아닌 프론트엔드용이므로 `get()` 은 기본적으로 단순화된 light 버전을 가져와 Leaflet·MapLibre·deck.gl 등에 바로 투입할 수 있습니다.
- 데모 사이트(`web/` 폴더) 소스는 이 라이브러리를 그대로 사용합니다.

### 공통 규약 (v0.6.0+)

- 지도 parquet: 1975 년 이후 읍면동/시군구/시도 경계, EPSG:5179. 파이썬 `get()` 에서 `crs=` 옵션으로 다른 좌표계(예: EPSG:4326) 변환 가능.
- 검색·시계열 인덱스는 패키지에 embed 되지 않고 이 저장소의 `dist/data/` 에서 import 시 자동 다운로드 됩니다. 지도 데이터 수정(이름 정정 등)이 있을 때 `pip install -U` / `npm update` 없이도 최신 인덱스가 반영됩니다.
- 아직 개발 초기 단계라 실제 사용하면서 기능이 추가될 수 있습니다. 개선 건의는 이 Github 의 [Issue 게시판](https://github.com/vuski/admdongkor/issues) 에 올려주세요.
---

# 대한민국 행정동 경계(admdongkor)

- 저작권 관련 사항은 본 문서의 끝부분에 기술하였으니 참고하시기 바랍니다.
- 정확하고 자세한 행정동 경계가 필요한 분들은 아래의 링크들에서 다운받으시기 바랍니다.
  - https://business.juso.go.kr/jst/jstAddressDetailsSearch
    - 2022년 5월 부터 도로명주소지도 사이트에서도 행정동 경계를 배포하고 있습니다. 한 달에 한번 업데이트 됩니다.
    - 위의 링크에서 '구역의 도형'을 신청하여 받으면 됩니다.
    - 시도 별로 분할되어 있으며, TL_SCCO_GEMD 파일이 행정동 경계입니다.
    - 이 곳에서 배포하는 경계보다 훨씬 세밀하고 정확하며, 토폴로지 정합성도 거의 잘 맞춰져 있습니다.
    - 10자리 코드가 기재되어 있습니다.
  - https://www.vworld.kr/dtmk/dtmk_ntads_s001.do
    - 브이월드 디지털트윈국토 사이트에서도 행정동 경계를 받을 수 있습니다.
    - 위의 링크에서 '행정동'으로 검색하면 됩니다.
    - 6개월~1년에 한 번 정도 업데이트 되는 것 같습니다.
...
- 속성의 adm_cd 는 통계청에서 사용하는 8자리의 [한국행정구역분류코드]입니다.
- 속성의 adm_cd2 는 행정안전부 사용하는 10자리의 [행정기관코드]입니다.(2018.07.24 업데이트 파일부터 적용)
- 속성의 adm_cd8 는 통계청에서 사용하는 8자리의 새로운 [한국행정구역분류코드]입니다. (2022.01.01 업데이트 파일부터 적용 -> 2023.10.01 업데이트 이후로 7자리 adm_cd를 삭제하고 adm_cd8의 속성명을 adm_cd로 변경함)
- 속성의 adm_nm 은 통계청에서 사용하는 전국 행정동 이름입니다.
- 일부 지역들이 실제 행정동 경계와 잘 맞지 않습니다. 지속적인 수정이 필요합니다.
- 최신 버젼만 업로드하지 않고 변경 이력별로 지속적으로 남길 예정입니다.
- 업로드 된 지도들은 다음의 데모 페이지에서 조회해 볼 수 있습니다.
- https://admdongkor.vw-lab.com
...
- 최신 자료인 2001~2023년에 존재하는 모든 출장소, 읍면동, 시군구, 시도 중심점이 있습니다. 출장소는 위치를 별도로 찾아서 입력해두었습니다.
...
│   └── src/admdongkor/          # 코드만 배포 (v0.6.0~ 인덱스 parquet 은 별도)
├── dist/data/                   # 인덱스·타임라인·shape_pairs parquet + manifest.json
│                                # (lib 이 import 시 GitHub raw 에서 자동 다운로드)
...
- ver20260701
  - 2026년 7월 1일 광주광역시와 전라남도가 전남광주통합특별시로 통합되었습니다. 이에 따라 시도 및 시군구 코드도 수정되었습니다. 시도코드 앞 2자리 대체만으로 매칭되지 않는 시군구 코드들이 있음에 유의하시기 바랍니다. 시군구와 읍면동 이름은 변경되지 않았습니다.
  - 2026년 7월 1일 인천광역시 일부 자치구가 통합 및 분할되었습니다. 중구와 동구가 영종구와 제물포구로 변경(일부 분할, 일부 통합)되었습니다. 영종도 및 일부 도서가 영종구로 독립, 나머지 지역이 제물포구로 통합되었습니다. 서구가 서해구 및 검단구로 분할되었습니다.
  - 2026년 7월 1일 경기도 안양시만안구 안양8동이 명학동으로, 안양9동이 병목안동으로 명칭변경되었습니다.

- ver20260401
  - 2026년 3월 25일 충청북도 음성군 대소면이 대소읍으로 승격되었습니다.

- ver20260201
  - 2026년 1월 1일 인천광역시 중구 운서동이 운서1동과 운서2동으로 분동되었습니다.
  - 2026년 1월 2일 경기도 용인시 처인구 양지면이 양지읍으로 승격되었습니다.
  - 2026년 2월 1일 경기도 화성시에 일반구 4개구(만세구, 효행구, 병점구, 동탄구)가 신설되면서 명칭이 전반적으로 변경되었습니다. (ex. 경기도 화성시 우정읍 → 경기도 화성시 만세구 우정읍)

- ver20251231
  - 2025년 10월 20일 인천광역시 서구 아라동이 아라1동과 아라2동으로 분동되었습니다.
  - 2025년 10월 27일 전북특별자치도 전주시 덕진구 송천2동이 송천2동과 송천3동으로 분동되었습니다.

- ver20251001
  - 2025년 9월 1일 경상북도 경주시 중부동이 폐지되면서 인접한 황오동으로 편입되었습니다.

- ver20250701
  - 2025년 7월 1일 경기도 안양시 만안구 박달1동이 박달동으로, 박달2동이 호현동으로 명칭변경되었습니다.
  - 2025년 7월 1일 경기도 용인시 처인구 유림동이 유림1동과 유림2동으로 분동되었습니다.
  - 2025년 7월 1일 서울특별시 동대문구 용신동이 신설동과 용두동으로 분동되었습니다.
...
- 연중 지속적으로 이루어지는 행정구역 변경은 행정안전부 홈페이지에 게시됩니다.
  ...
  - [행정안전부 홈페이지](http://www.mois.go.kr/) → [업무안내 → 자치혁신실 → 주민등록 및 인감] 게시판에 변경내용이 있습니다.
...
- **코드**: [LICENSE](LICENSE) — MIT License. `js/`, `lib/`, `scripts/`, `web/` 등 소스 코드에 적용.
- **데이터**: [LICENSE-DATA](LICENSE-DATA) — Creative Commons Attribution 4.0 (CC BY 4.0). 원자료는 통계청 SGIS 의 공공누리 제1유형(출처표시) 으로 개방된 행정동 경계이며, 이 출처표시 의무는 가공 여부와 무관하게 승계됩니다. `parquet/`, `ver*/*.geojson`, `dist/data/` 등 모든 경계 데이터 파일에 적용.

데이터를 이용·재배포할 때 SGIS 출처표시와 본 저장소(`vuski/admdongkor`) 표기를 반드시 보존해 주시기 바랍니다. 자세한 출처 표기 문구는 [LICENSE-DATA](LICENSE-DATA) 의 §3 을 참고하세요.

## 저작권 관련 (배경)

- 이 github에서 배포하는 행정동 경계 데이터는 통계청 통계지리정보서비스(SGIS, sgis.kostat.go.kr)에서 개방한 행정동 경계를 바탕으로 수정·보정하고 시계열(1975~현재)로 확장한 것입니다.
  - SGIS 의 행정동 경계는 [공공누리 제1유형(출처표시)](https://www.kogl.or.kr/info/licenseType1.do) 으로 개방되어 있습니다. 즉 **출처를 표시하면 상업적 이용을 포함한 자유로운 이용·변형·재배포가 허용**됩니다.
  - 이 저장소를 처음 시작할 때(2017년경) 작성한 안내에는 "별다른 저작권에 관한 사항이 없다"고 적혀 있었으나, 이후 확인 결과 SGIS 는 명시적으로 공공누리 1유형을 적용하고 있어 안내를 정정합니다.
- 본 저장소의 가공물(경계 보정, 시계열 확장, 코드 매칭 인덱스 등)은 CC BY 4.0 으로 배포합니다. SGIS 공공누리 1유형의 출처표시 의무는 가공 여부와 무관하게 그대로 승계됩니다 — 자세한 내용은 [LICENSE-DATA](LICENSE-DATA) 참고.
- 출처표시 의무 외에 별도의 추가 제한은 두지 않습니다.
  - 이용 시 본 저장소 주소(https://github.com/vuski/admdongkor)도 함께 표시해 주시길 권장합니다.
  - 이는 수정된 부분에 대한 저작권을 주장하기 위함이 아니라, 자료에 문제나 틀린 부분을 발견한 분이 이 곳을 찾아올 수 있도록 하기 위함입니다.
