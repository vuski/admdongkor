...
# 2-1. 코드로 찾기 — 숫자만 넣으면 자동으로 코드 검색
adk.find("1111051500")                    # 청운효자동 (행안부 10자리)
adk.find("11110")                         # 종로구 + 그 하위 읍면동 전부 (prefix)
adk.find("11110", exact=True)             # 종로구만 (자릿수 완전일치)
adk.find_offices("2920083000")            # 출장소 — 지도 없음, 코드 조회 전용
...
### `find(name, level=None, exact=False, year=None, by=None) -> FindResult`

행정구역명 substring 검색 (NFC 정규화, 대소문자·공백 무시) **또는 코드 검색**.

**숫자로만 이루어진 쿼리는 자동으로 코드 검색**이 된다 — 행정구역명 중 숫자만인
것은 없어서 이름 검색과 충돌하지 않는다. `by="name"` / `by="code"` 로 강제 가능.
...
`"11"` → 시도 11 + 시군구 `11xxx` + 읍면동 `11xxxxxxxx` 전부. `level=` 로 좁힌다.
매칭 대상은 `code`(행안부) / `code7` / `code8`(통계청) 셋 다이므로 통계청 코드를
들고 있어도 찾을 수 있다 (1975~85 는 행안부 코드가 없어 통계청 7자리가 유일한 키).

같은 코드가 시점에 따라 다른 지역을 가리킬 수 있다 — 예를 들어 통계청 `1101053`
은 1975 에 누상동, 1980 부터 사직동이다. 시계열 인덱스이므로 둘 다 반환된다.

**계층 검색** — 이름 검색일 때 공백 토큰 수에 따라 자동 level 필터:
...
### `find_offices(query, exact=False, by=None) -> DataFrame`

출장소(出張所) 조회. **코드를 넣었을 때 그게 어디인지 알려주는 용도.**

출장소는 행안부 행정동 코드 체계에는 존재하지만 **경계 지도가 없다** (geojson/shp
에 나오지 않음). 그래서 `get()` 으로 지도를 받을 수 없고 `find()` 결과에도 포함되지
않는다. 코드 조회만 별도 경로로 제공한다.

`version_key` 대신 `created` / `abolished` (YYYYMMDD 문자열) 로 유효 기간을 표현한다.
`abolished` 가 NA 면 현존.

```python
adk.find_offices("2920083000")   # 광주 광산구 임곡출장소 (1995-01-01 ~ 1998-10-15 말소)
adk.find_offices("28")           # 인천 소속 출장소 전부 (prefix)
adk.find_offices("영종")          # 이름으로 — 인천 영종출장소의 변천 이력
```

**반환 컬럼:** `code, name, sggnm, sidonm, sggcd, sidocd, level, created, abolished`

2026-07 기준 392건 (현존 75 / 말소 317). `level` 은 코드 뒤 5자리가 `00000` 이면
`"sgg"`, 아니면 `"emd"`.

> 데이터 버전 `2026.08.02` 부터 제공. 그 이전 캐시를 쓰는 경우 (자동 업데이트를
> 껐거나 `ADMDONGKOR_DATA_DIR` 로 옛 사본 고정) 예외 대신 빈 결과를 반환한다.
