 ...
 *
 * ★ 울릉도·독도는 2026-09-03 에 **경상북도 옆(남서쪽)** 으로 재배치했다.
 * 종전 위치는 두 섬이 경북 bbox 밖 북동쪽으로 튀어나가 전체 지도 bbox 를
 * 넓혀 버렸다. 새 값은 `data-explorer` 의 확정 배치
 * `backend/processed/boundary/parquet/adk229_sgg_s027_pulled_20250630.parquet`
 * 에서 part 별 좌표차로 역산했다 (표준편차 0.000000 — 순수 평행이동).
 */
const ISLANDS: IslandBox[] = [
  ...
  },
  {
    ...
    offset: [-98238.1, -70271.7],
  },
  {
    name: "독도",
    bbox: [1380937.2, 1920261.8, 1394640.7, 1931525.0],
    offset: [-172453.8, -63965.5],
  },
];

/**
 * 당겨온 표시 지시선 — 둔각 꺾은선(3점) 6개. **EPSG:5179**.
 ...
 *
 * ★ 울릉도·독도 지시선도 재배치와 함께 옮겼다. 다만 두 선의 이동량은
 * **섬의 offset 과 다르다** — 둘 다 (+11206.9, -55000.0) 으로 같이 움직였다.
 * 지시선은 섬을 따라가는 게 아니라 "본토 → 옮긴 자리" 를 잇는 별개 배치라
 * 섬 offset 을 그대로 적용하면 안 된다. 값은 확정 파일
 * `adk229_connector_s027_pulled_20250630.parquet` 에서 그대로 옮겨 적었다.
 */
const CONNECTOR_LINES: { island: IslandName; coords: Position[] }[] = [
  ...
  {
    island: "독도",
    coords: [[1219057.2, 1878366.4], [1207334.2, 1868556.0], [1211099.4, 1851110.9]],
  },
  {
    island: "울릉도",
    coords: [[1201460.5, 1900410.6], [1186064.1, 1884390.8], [1190861.2, 1861154.5]],
  },
];
